...
var readFileSync = sinon.stub().returns(fs.readFileSync(path.resolve(__dirname, '../data/cmm.conf')));
var BitcoinService = proxyquire('../../lib/services/bitcoind', {
  fs: {
    readFileSync: readFileSync
  }
});
var defaultBitcoinConf = fs.readFileSync(path.resolve(__dirname, '../data/default.cmm.conf'), 'utf8');
...
describe('Bitcoin Service', function() {
  ...
  describe('#_loadSpawnConfiguration', function() {
    ...
    it('will parse a cmm.conf file', function() {
      ...
      bitcoind.options.spawn.datadir = '/tmp/.cmm';
      var node = {};
  ...
  describe('#_checkConfigIndexes', function() {
    ...
    it('should warn the user if reindex is set to 1 in the cmm.conf file', function() {
      var bitcoind = new BitcoinService(baseConfig);
      var config = {
        txindex: 1,
        addressindex: 1,
  ...
  describe('#_getNetworkConfigPath', function() {
    ...
    it('will get default rpc port for testnet', function() {
      ...
      var bitcoind = new BitcoinService(config);
      bitcoind._getNetworkConfigPath().should.equal('testnet3/cmm.conf');
    });
    it('will get default rpc port for regtest', function() {
      bitcore.Networks.enableRegtest();
      var config = {
      ...
      var bitcoind = new BitcoinService(config);
      bitcoind._getNetworkConfigPath().should.equal('regtest/cmm.conf');
    });
  });
  ...
  describe('#_spawnChildProcess', function() {
    ...
    it('will include network with spawn command and init zmq/rpc on node', function(done) {
      ...
      bitcoind.spawn.configPath = 'testdir/cmm.conf';
    ...
    it('will respawn bitcoind spawned process', function(done) {
      ...
      bitcoind.spawn.configPath = '/tmp/bitcoin/cmm.conf';
      bitcoind.spawn.config = {};
