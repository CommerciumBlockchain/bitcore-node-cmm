...
Bitcoin.prototype._loadSpawnConfiguration = function(node) {
  ...
  var configPath = path.resolve(spawnOptions.datadir, './cmm.conf');

  log.info('Using cmm config file:', configPath);
...
Bitcoin.prototype._checkConfigIndexes = function(spawnConfig, node) {
  ...
  $.checkState(
    ...
    '"zmqpubrawtx" and "zmqpubhashblock" are expected to the same host and port in cmm.conf'
  );
...
Bitcoin.prototype._getNetworkConfigPath = function() {
  var networkPath;
  if (this.node.network === bitcore.Networks.testnet) {
    networkPath = 'testnet3/cmm.conf';
    if (this.node.network.regtestEnabled) {
      networkPath = 'regtest/cmm.conf';
    }
  }
  return networkPath;
};
...
Bitcoin.prototype._stopSpawnedBitcoin = function(callback) {
  ...
  function stopProcess() {
    fs.readFile(pidPath, 'utf8', function(err, pid) {
      ...
      try {
        log.warn('Stopping existing spawned cmm process with pid: ' + pid);
        self._process.kill(pid, 'SIGINT');
      } catch(err) {
        if (err && err.code === 'ESRCH') {
          log.warn('Unclean cmm process shutdown, process not found with pid: ' + pid);
          return callback(null);
        } else if(err) {
          return callback(err);
        }
  ...
  self._stopSpawnedBitcoin(function(err) {
    ...
    self.spawn.process.once('exit', function(code) {
      if (!self.node.stopping) {
        ...
        log.warn('Restarting cmm child process in ' + self.spawnRestartTime + 'ms');
        setTimeout(function() {
